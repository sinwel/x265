...
void x265_param_default(x265_param *param)
{
    ...
    param->bOpenGOP = 0;
    // default disable b frame
    param->bframes = 0;                             // 4
    // default disable lookahead
    param->lookaheadDepth = 0;                      // 20
    // default disable b frame
    param->bFrameAdaptive = X265_B_ADAPT_NONE;      // X265_B_ADAPT_TRELLIS
    param->bBPyramid = 0;                           // 0
    ...
    param->rc.qp = 32;
    // default not aq mode
    param->rc.aqMode = X265_AQ_NONE;                // X265_AQ_VARIANCE
    param->rc.aqStrength = 1.0;
    // default disable cuTree
    param->rc.cuTree = 0;                           // 1
...
#define CHECK(expr, msg) check_failed |= _confirm(param, expr, msg)
	...
	CHECK(param->meRangeX < 0,
		"Search Range must be more than 0");
	CHECK(param->meRangeX >= 32768,
		"Search Range must be less than 32768");
	CHECK(param->meRangeY < 0,
		"Search Range must be more than 0");
